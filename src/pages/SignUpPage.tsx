...
type ErrorForm = Partial<Record<keyof SignUpForm, string>>;
...
const getBirthDateFromRRN = (rrn: string): string => {
  ...
  // 성별코드로 19/20/21세기 구분
  let fullYear = '';
  if (genderCode === '1' || genderCode === '2') fullYear = String(1900 + year);
  else if (genderCode === '3' || genderCode === '4') fullYear = String(2000 + year);
  else if (genderCode === '5' || genderCode === '6') fullYear = String(1900 + year); // 외국인
  else if (genderCode === '7' || genderCode === '8') fullYear = String(2000 + year); // 외국인
...
const SignUpPage = () => {
  ...
  const [error, setError] = useState<ErrorForm>({}); // 에러 메시지
